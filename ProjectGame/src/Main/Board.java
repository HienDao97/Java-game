...
//	       System.out.println(hero.getLive());
		...
		private void updateFires() {
		   ...
		   for (int i=0 ; i< fires.size();i++) {
	           ...
	           else { fires.remove(i); } // xoa vien dan khi ham move
		   }
		...
		private void updateHero() {	
		   if (hero.getTontai()) { hero.move(); }
		   if (!hero.getTontai()) ingame=false; // --> neu co mang -> tru mang
		}
		...
		public void paintComponent(Graphics g) { 
			...
			// ve map
			ImageIcon treeImage=new  ImageIcon("res/textures/img/cay1.png");
		//	ImageIcon wallImage=new  ImageIcon("res/textures/wall.png");
		//	ImageIcon groundImage=new  ImageIcon("res/textures/ground.png");
			ImageIcon grassImage = new ImageIcon("res/textures/img/co.png");
			ImageIcon datImage = new ImageIcon("res/textures/img/dat.png");
			ImageIcon thungImage = new ImageIcon("res/textures/img/thung.png");
			ImageIcon nuocImage = new ImageIcon("res/textures/img/nuoc.png");
			ImageIcon cauImage = new ImageIcon("res/textures/img/cau.png");
			...
			for (int y=0;y<20;y++)
				for(int x=0;x<20;x++) {
					if(m.getMap(x, y).equals("1")) {
						...
						g.drawImage(new Nuoc(x,y,30,30,nuocImage.getImage()).getImage(), x*30, y*30, null);
					}else if(m.getMap(x, y).equals("3")) {
						g.drawImage(new Thung(x,y,30,30,thungImage.getImage()).getImage(), x*30, y*30, null);
					}else if(m.getMap(x, y).equals("4")) {
						g.drawImage(new Cau(x,y,30,30,cauImage.getImage()).getImage(), x*30, y*30, null);
					}else if(m.getMap(x, y).equals("5")) {
						g.drawImage(new Dat(x,y,30,30,datImage.getImage()).getImage(), x*30, y*30, null);
					}else if(m.getMap(x, y).equals("0")) {
						g.drawImage(new Grass(x,y,30,30,grassImage.getImage()).getImage(), x*30, y*30, null);
					}
...
//				intro.init();
			//	private Image backgroundImage = ImageIO.read(new File("res/textuers/img/thanos.png"));
		    ...
		    if (ingame) {       
		       ...
		       if (monsters.isEmpty()  ) {
		    	  ...
		    	  if (boss_appared == false) boss_appared =true;
		       }
		       else
